...
# KITEH TODO Change these urls to kitehscoin.io and add a depends subdomain which will do what this does
# Dependencies
...
SIGNER=
VERSION=
url=https://github.com/Kitehcoin/Kitehcoin-core
proc=2
mem=2000
...
Standalone script to perform the gitian build of Kitehcoin Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/Kitehcoin/kitehcoin
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/kitehcoin-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    ...
    git clone https://github.com/Kitehcoin/kitehcoin-detached-sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/kitehcoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean kitehcoin git directory because of old caching
    if [ -d inputs/kitehcoin/ ]; then
        echo "Cleaning Kitehcoin directory..."
        rm -rf inputs/kitehcoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit kitehcoin="$COMMIT" --url kitehcoin="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
