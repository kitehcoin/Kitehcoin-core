...
#if defined(NDEBUG)
# error "Kitehcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Kitehcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetKitehcoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Kitehcoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("kitehcoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Kitehcoin: BIP30 has been active since inception
    ...
    // Kitehcoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetKitehcoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
    ...
    if (!IsValidDestination(destDeveloperFeeAddress)) {
        LogPrintf("IsValidDestination: Invalid kitehcoin address %s \n", GetDeveloperFeeAddress);
    }
    ...
    CAmount nSubsidy = GetKitehcoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Kitehcoin: Disallow AuxPow blocks before it is activated.
    ...
    // Kitehcoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Kitehcoin: We probably want to disable this
    int nLockTimeFlags = 0;
