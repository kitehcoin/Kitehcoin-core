...
const char * const BITCOIN_CONF_FILENAME = "kitehcoin.conf";
const char * const BITCOIN_PID_FILENAME = "kitehcoind.pid";
...
#else
    const char* pszModule = "kitehcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Kitehcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Kitehcoin";
#else
    // Unix
    return pathRet / ".kitehcoin";
#endif
#endif
}
