...
Below are some notes on how to build Kitehcoin Core for Windows.
...
Kitehcoin GitHub repository](https://github.com/Kitehcoin/kitehcoin/)).
...
Note that for WSL v1 the Kitehcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/kitehcoin, AND not under, for example, /mnt/d/kitehcoin. 
...
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Kitehcoin's dependencies):
...
way. This will install to `c:\workspace\kitehcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/kitehcoin
