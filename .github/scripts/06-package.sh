...
cd ${GITHUB_WORKSPACE}
PKGVERSION=`grep "PACKAGE_VERSION" src/config/kitehcoin-config.h | cut -d\" -f2`
VERSION="${PKGVERSION}"
...
if [[ ${OS} == "windows" ]]; then
    ...
    for rmfile in detach-sig-create.sh win-codesign.cert kitehcoin-cli.exe kitehcoin-qt.exe kitehcoind.exe; do
        if [[ -e ${rmfile} ]]; then
            rm -f ${rmfile}
        fi
    done
    ...
    if [[ -e ${GITHUB_WORKSPACE}/dist/Kiteh-Qt.app/Contents/MacOS/install_cli.sh ]]; then
        chmod +x ${GITHUB_WORKSPACE}/dist/Kiteh-Qt.app/Contents/MacOS/install_cli.sh
    fi
    ...
    ${GITHUB_WORKSPACE}/depends/x86_64-apple-darwin14/native/bin/dmg dmg "KITEH-Core.dmg" ${RELEASE_LOCATION}/${DISTNAME}-osx-unsigned.dmg
    ...
    if [[ -e ${RELEASE_LOCATION} ]]; then
        ...
        cd ${STAGE_DIR}
        cp -Rf ${DISTNAME}/bin/kitehcoind .
        cp -Rf ${DISTNAME}/bin/kitehcoin-cli .
    else
        echo "release directory doesn't exist"
    fi
